...
const CustomFieldsModal = ({ isOpen, onClose }) => {
  ...
  const handleFieldNameChange = useCallback((e) => setFieldName(e.target.value), []);
  const handleFieldTypeChange = useCallback((e) => setFieldType(e.target.value), []);

  const handleAddField = useCallback(() => {
    if (!fieldName.trim() || customFields.some(field => field.name === fieldName)) {
      setError("Invalid or duplicate field name!");
      return;
    }

    dispatch({ type: ACTIONS.ADD_CUSTOM_FIELD, payload: { name: fieldName, type: fieldType } });
    setFieldName("");
    setError("");
  }, [fieldName, fieldType, customFields, dispatch, ACTIONS]);

  const handleRemoveField = useCallback((name) => {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal">
        ...
        <button className="modal__button modal__button--add" onClick={handleAddField}>Add Field</button>
        ...
        <ul className="modal__list">
          {customFields.map(field => (
            <li key={field.name} className="modal__list-item">
              ...
              <button className="modal__button modal__button--delete" onClick={() => handleRemoveField(field.name)}>❌</button>
            </li>
          ))}
        </ul>

        <button className="modal__button modal__button--close" onClick={onClose}>Close</button>
      </div>
    </div>
  );
};
